...
public class Fork6 extends BusComponent {

    private BooleanProperty busToSingle;
    private ToggleGroup toggle;
    private Pin bus, singles[];

    public Fork6(ControlMain control) {
        super(control, false);
    ...
    public Fork6(ControlMain control, Element data) {
        this(control);
        confirm();
        readXML(data);
    }
    @Override protected HashSet<Pin> initPins() {
        ...
        bus = new Pin(this, true, 6 * cap, 0, 3);
        pins.add(bus);
    ...
    @Override public void simulate() {
        ...
        if (busToSingle.get()) {
            ...
            for (int i = 0; i < 6; i++) singles[i].put(Arrays.copyOfRange(bus, i * cap, (i+1) * cap));
        } else {
            LogicLevel[] res = new LogicLevel[6 * cap];
            for (int i = 0; i < 6; i++) System.arraycopy(singles[i].get(), 0, res, i * cap, cap);
            bus.put(res);
        }
    }
