...
public class HalfAdder extends Component {
    ...
    public HalfAdder(ControlMain control) {
        super(control);
        DoubleProperty three = new SimpleDoubleProperty(3.5);
        DoubleProperty six = new SimpleDoubleProperty(5.5);

        // position label for Pin inB
        Label lblB = (Label) getRoot().lookup("#lblB");
        lblB.layoutYProperty().bind(three.subtract(lblB.heightProperty()));

        // position label for Pin outS
        Label lblS = (Label) getRoot().lookup("#lblS");
        lblS.layoutXProperty().bind(six.subtract(lblS.widthProperty()));

        // position label for Pin outC
        Label lblC = (Label) getRoot().lookup("#lblC");
        lblC.layoutXProperty().bind(six.subtract(lblC.widthProperty()));
        lblC.layoutYProperty().bind(three.subtract(lblC.heightProperty()));
    }
